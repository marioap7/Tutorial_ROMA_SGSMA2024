...
August 11th, 2021
	...
	def click_btn32(self):
		...
		if self.Check_Data(2, 34, 56, 7, 9) == True:
			...
			M_res = M_rts = modes = L_mod = list_signals = t_vec = y_vec = dt = N = t_aprx = med = can = None
	...
	def click_btn7(self):
		...
		sheet_1['A3'] 	= 'User: '
		sheet_1['A3'].alignment = Alignment(horizontal = 'right')
		...
		sheet_1['B3'] 	= 'Rodrigo David Reyes de Luna'
		...
		for i_signal in range(M_res.shape[1]):
			...
			for i in range(M_res.shape[0]):
				if freq[i] > 0:
					...
					sheet_1.cell(23 + j + jj , 10).value = str(round(roots[i].real,2)) +' + j'+ str(round(roots[i].imag,2))
		...
		for i in range(max(L_mod)):
			if freq[i] > 0:
				...
				row_val = ('✓', m, osc_mod, round(freq[i]	, 4),
											...
											round(-damprat[i], 4),
											round(ang[i]	, 4),
											complex(round(roots[i].real, 3), round(roots[i].imag, 3)),
											round(enrgy[i]	, 4))
